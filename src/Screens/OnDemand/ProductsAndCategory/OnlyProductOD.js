...
export default function OnlyProductOD({route, navigation}) {
...
console.log(data,'testdatadata')
  ...
  const listHeaderComponent2 = () => {
    const imageUrl = getImageUrl(
      ...
      categoryInfo?.banner?.proxy_url || categoryInfo?.image?.proxy_url,
      // data?.item?.banner.image_path ||
      categoryInfo?.banner?.image_path || categoryInfo?.image?.image_path,
      '800/400',
    );
    ...
    return (
      <View
        ...
        <View style={{marginBottom: moderateScaleVertical(16)}}>
          ...
          <View
            ...
            <View style={styles.horizontalView}>
              ...
              {console.log(categoryInfo, 'categoryInfocategoryInfo')}
              <View style={{flexDirection: 'row', alignItems: 'center'}}>
                <View
                  ...
                  <Text>
                    {' '}
                    {Number(categoryInfo?.product_avg_average_rating || categoryInfo?.rating || 0.0).toFixed(
                      1,
                    )}
                  </Text>
                </View>
        ...
        { !isEmpty(cloneSectionList)&& <FlatList
            horizontal
            showsHorizontalScrollIndicator={false}
            data={cloneSectionList}
            renderItem={({item,index}) => (
              <TouchableOpacity onPress={() => onPressCategoryOption(index)} style={{width:moderateScale(width/5.5),alignItems:'center',marginHorizontal:moderateScale(2),marginVertical:moderateScaleVertical(10)}}>
                <Image resizeMode='contain' style={{height:moderateScale(40),width:moderateScale(40)}} source={imagePath.icCategory}/>
                <Text style={{textAlign:'center',marginTop:moderateScaleVertical(10),fontSize:textScale(10)}}>{item?.translation[0]?.name}</Text>
              </TouchableOpacity>
            )}
          />}
      </View>
  ...
  const getAllProductsByCategoryId = (pageNo, filterValue) => {
    ...
    let apiUri;
    if (filterValue) {
      apiUri = `/${productWithCategoryId}?page=${pageNo}&product_list=${
  ...
  const onPressCategoryOption = index => {
    setActiveIdx(index);
    sectionListRef.current.sectionList.current.scrollToLocation({
      sectionIndex: index,
      itemIndex: 0,
      ...
      if (selectedAllProductDataForAppointment?.is_slot_from_dispatch) {
        ...
        try {
          ...
          if (!!res && !!res?.data && isEmpty(res?.data)) {
            ...
            setSelectedAppointmentSlot({});
            alert(
              'Please choose a different date as the selected slots are not available at the moment.',
            );
            return;
          }
          if (res) {
      ...
      <View style={{flex: 1}}>
        <View
          ...
          <View style={{flex: 1}}>
            ...
            {console.log(
              cloneSectionList,
              'cloneSectionListcloneSectionList',
              tagFilteredData,
            )}
            {!!categoryInfo?.is_show_products_with_category ? (
              <SectionList
          ...
          {!isVisibleModal && (
            <GradientCartViewOnDemand
              ...
              textStyle={{color:colors.textGrey,fontSize:16,textAlign:'left'}}
              textStyle2={{textAlign:'left'}}
              btnText2={ `${CartItems?.data?.item_count} ${ CartItems?.data?.item_count == 1
                      ? strings.ITEM
                      : strings.ITEMS}`}
              btnText={
               `${tokenConverterPlusCurrencyNumberFormater(
