...
export class CircleList extends PureComponent {
  ...
  constructor(props) {
    ...
    const elementCountWithMin = Math.max(elementCount, 6);
    ...
    this._panResponder = PanResponder.create({
      ...
      onPanResponderMove: (_, gestureState) => {
        ...
        const {breakpoints, displayData, rotationIndex, scrolling, theta} =
          this.state;
        ...
        if (newRotationIndex !== rotationIndex) {
          ...
          const [insertionIndexLeft, insertionIndexRight] =
            this._getInsertionIndex(direction, 'ELEMENTS');
  ...
  _calcHeight = () => {
    ...
    return ((6 / elementCount) * 1.8 * radius) / 2;
  };
  ...
  _getDataIndex = (direction) => {
    ...
    if (direction === 'LEFT') {
      ...
      return incrementedIndex >= length
        ? incrementedIndex - length
        : incrementedIndex;
    }
    if (direction === 'RIGHT') {
  ...
  _getInsertionIndex = (direction, type) => {
    ...
    if (direction === 'RIGHT') {
      ...
      return [
        decrementedIndexLeft < 0
          ? length + decrementedIndexLeft
          : decrementedIndexLeft,

        decrementedIndexRight < 0
          ? length + decrementedIndexRight
          : decrementedIndexRight,
      ];
    }
  };
  ...
  _getOffsetData = (data, elementCount) => {
    ...
    return [...data.slice(0, elementCount)];
  };
  ...
  _getTransforms = (index) => {
    ...
    const thetaOffset =
      2 * PI * index + (this.rotationOffset + initialRotationOffset);
    const translateX = radius * cos(index * theta + thetaOffset);
    const translateY =
      (1 - flatness) * radius * sin(index * theta + thetaOffset) +
      (1 - flatness) * radius;
  ...
  _getVisibleElements = () => {
    ...
    const leftBound = -visibilityPadding - 1;

    const leftBoundAdjusted = leftBound < 0 ? length + leftBound : leftBound;

    const rightBound = -visibilityPadding + 1;

    const rightBoundAdjusted =
      rightBound > length ? rightBound - length : rightBound;
